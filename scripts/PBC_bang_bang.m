%% SIMPLE TRAJECTORY USING BANG-BANG CONTROL PROFILE - PBC NUMERIC
...
resultsPath = "../results/PBC/" + path;
plot_dir = "../plots/PBC/" + path;

if TRAJ_A
    resultsPath = strcat(resultsPath, "PBC_bb_A.mat");
    traj = "A";
else
    resultsPath = strcat(resultsPath, "PBC_bb_B.mat");
    traj = "B";
end
...
fprintf('\n=== SIMULAZIONE PBC CON TRAIETTORIA BANG-BANG ===\n');
fprintf('Integrando equazioni dinamiche...\n');

tic;
[t_sim, x_sim] = ode45(@(t, x) robot_dynamics_bang_bang(t, x, q0, qf, T_final, @PBC_numeric, ...
...
for i = 1:length(tau_sample_idx)
    ...
    tau_computed(i,:) = PBC_numeric(q_sim(:,idx), qd_sim(:,idx), q_d, qd_d, qdd_d, controller_gains, uncertain_params);
end
...
results = struct();
results.controller = 'PBC_numeric_bang_bang';
