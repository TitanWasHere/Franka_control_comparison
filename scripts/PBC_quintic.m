%% SIMPLE TRAJECTORY USING PASSIVITY-BASED CONTROL - PURE NUMERIC VERSION

clear; clc; close all;

fprintf('=== QUINTIC TRAJECTORY - PBC ===\n\n');
...
if TRAJ_A
    resultsPath = strcat(resultsPath, "PBC_quintic_A.mat");
    traj = "A";
else
    resultsPath = strcat(resultsPath, "PBC_quintic_B.mat");
    traj = "B";
end
...
tic;
[t_sim, x_sim] = ode45(@(t, x) robot_dynamics_numeric(t, x, q0, qf, T_final, @PBC_numeric, ...
...
for i = 1:length(t_sim)
    ...
    tau_computed(i,:) = PBC_numeric(q_sim(:,i), qd_sim(:,i), q_d, qd_d, qdd_d, controller_gains, uncertain_params);
end
